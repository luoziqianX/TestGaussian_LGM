...
class MVAttention(nn.Module):
    def __init__(
            ...
            residual: bool = True,
            skip_scale: float = 1,
            num_frames: int = 21,  # WARN: hardcoded!
    ):
        super().__init__()
...
class ControlUnet(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.input_hint_block = zero_module(
            nn.Conv2d(hint_channels, down_channels[0], kernel_size=3, padding=1)
        )
...
class ControlNetwork(UNet):
    def forward(self, x, control=None, only_mid_control=False):
        xss = []
        with torch.no_grad():
            x = self.conv_in(x)
            xss.append(x)
